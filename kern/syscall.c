	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((srcva >= (void *)UTOP) || (dstva >= (void *)UTOP) || (ROUNDDOWN(srcva, PGSIZE) != srcva) || (ROUNDDOWN(dstva, PGSIZE) != dstva))
		return -E_INVAL;
	pte_t *pte;
	struct PageInfo *src_page = page_lookup(src_env->env_pgdir, srcva, &pte);
	if (!src_page)
		return -E_INVAL;
	if (((perm | PTE_U | PTE_P) != perm) || ((perm & (PTE_U | PTE_P | PTE_AVAIL | PTE_W)) != perm))
		return -E_INVAL;
	if ((perm & PTE_W) && ((*pte | PTE_W) != *pte))
		return -E_INVAL;
	if (page_insert(dst_env->env_pgdir, src_page, dstva, perm) == -E_NO_MEM)
		return -E_NO_MEM;
	return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (srcva && (unsigned)srcva < UTOP) {
		if (ROUNDDOWN((unsigned)srcva, PGSIZE) != (unsigned)srcva)
			return -E_INVAL;
		int check_perm = PTE_U | PTE_P;
		if ((perm & check_perm) != check_perm || (perm & (~(check_perm | PTE_W | PTE_AVAIL))))
			return -E_INVAL;
		if (perm & PTE_W)
			check_perm |= PTE_W;
		int ret_1 = user_mem_check(curenv, srcva, PGSIZE, check_perm);
		if (ret_1 < 0)
			return -E_INVAL;
		if (e->env_ipc_dstva) {
			...
			if (!page_info)
				return -E_INVAL;
			int ret_3 = page_insert(e->env_pgdir, page_info, e->env_ipc_dstva, perm);
			if (ret_3 < 0)
				return -E_NO_MEM;
			e->env_ipc_perm = perm;
		}
...
sys_ipc_recv(void *dstva)
{
	...
	if (dstva && (unsigned)dstva < UTOP && (ROUNDDOWN((unsigned)dstva, PGSIZE) != (unsigned)dstva))
		return -E_INVAL;
...
static int sys_packet_try_send(void *data, int len) {
	return e1000_transmit(data, len);
}

static int sys_packet_try_recv(void *addr, size_t *len) {
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_packet_try_recv:
		return sys_packet_try_recv((void *)a1, (size_t *) a2);
		break;
	default:
		return -E_INVAL;
	}
